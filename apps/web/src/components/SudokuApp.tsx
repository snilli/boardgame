...
import { useAutoGamePause } from '@app/hooks/useAutoGamePause'
...
import WebAudioLoFiPlayer from './WebAudioLoFiPlayer'
...
export default function SudokuApp() {
	...
	const { formattedTime } = useGameTimer({
		...
		totalPausedDuration: gameState?.totalPausedDuration || 0,
	...
	// Auto-pause when switching tabs or minimizing window
	useAutoGamePause({
		gameMode,
		isPaused: gameState?.isPaused || false,
		pauseGame: () => {
		...
		resumeGame: () => {
			if (gameMode === 'paused') {
				pause() // pause() actually toggles pause/resume
			}
		},
	})
	...
	return (
		<div className="relative min-h-screen w-screen overflow-hidden bg-gradient-to-br from-indigo-900 via-purple-900 to-pink-900">
			{/* LoFi Music Player - Web Audio API */}
			<WebAudioLoFiPlayer isGamePlaying={gameMode === 'playing'} />
			...
			<div className="fixed top-0 right-0 left-0 z-30 border-b border-white/10 bg-black/20 backdrop-blur-lg">
				<div className="mx-auto flex max-w-7xl items-center justify-between px-2 py-3 sm:px-4 sm:py-4 md:px-6">
					...
					<div className="flex gap-1 sm:gap-2">
						<AudioSettings gameMode={gameMode} />
						<motion.button
							onClick={toggleNotes}
							className={cn(
					...
					<div className="grid grid-cols-5 gap-3 sm:gap-4">
						{SUDOKU_NUMBERS.map((num) => (
							<motion.button
								...
								onClick={() => placeNumber(num)}
								className={cn(
									'flex h-14 cursor-pointer items-center justify-center rounded-xl border-2 text-lg font-bold transition-all duration-200 sm:h-16 sm:rounded-2xl sm:text-xl',
									highlightedNumber === num
						...
						<motion.button
							onClick={clearCell}
							className="flex h-14 cursor-pointer items-center justify-center rounded-xl border-2 border-red-400/50 bg-red-500/90 text-lg font-bold text-white hover:bg-red-600/90 sm:h-16 sm:rounded-2xl sm:text-xl"
							whileTap={{ scale: 0.95 }}
						>
